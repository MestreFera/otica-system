...
import { Plus, Search, ChevronRight, Package, Clock, CheckCircle, Truck, AlertTriangle, Link as LinkIcon, Activity, Upload, Download } from 'lucide-react';
...
export default function ClientsPanel() {
    ...
    const filtered = clients.filter(c => {
        ...
        // A raw lead is usually "Novo", has an AI status, but NO total_value and NO tso.
        // We add `&& c.telefone_cliente` or check that it was actually generated by the automation to be safer.
        // Actually, let's just make sure we only hide them if they truly look like just a chat interaction.
    ...
    function handleImportCSV(file) {
        setImporting(true);
        addToast({ type: 'info', message: 'Iniciando importação...', duration: 2000 });
        Papa.parse(file, {
            header: true,
            skipEmptyLines: true,
            complete: async (results) => {
                if (results.errors.length > 0 && results.data.length === 0) {
                    addToast({ type: 'error', message: 'Erro ao analisar CSV. Vefique o formato.' });
                    setImporting(false);
                    return;
                }
                const res = await clientService.bulkImportClients(unitId, results.data);
                if (res.success) {
                    addToast({ type: 'success', message: `${res.count} clientes importados com sucesso!`, duration: 5000 });
                    // Reload clients list
                    setLoading(true);
                    const data = await clientService.getClients(unitId);
                    setClients(data);
                    ...
                    addToast({ type: 'error', message: res.error || 'Erro na importação' });
                }
                setImporting(false);
            },
            error: () => {
                addToast({ type: 'error', message: 'Erro ao carregar o arquivo' });
                setImporting(false);
            }
        });
    }
    ...
    function handleExportCSV() {
        if (filtered.length === 0) {
            addToast({ type: 'warning', message: 'Nenhum dado para exportar.' });
            return;
        }
        ...
        const csv = Papa.unparse(filtered.map(c => ({
            ID: c.id,
            TSO: c.tso,
            NOME: c.name || c.client_name,
            TELEFONE: c.phone,
            STATUS: c.status,
            TIPO_LENTE: c.lens_type,
            ESTAGIO_FLUXO: c.etapa_fluxo,
            DATACRIACAO: new Date(c.created_at).toLocaleString('pt-BR')
        })));
        ...
        link.href = url;
        link.setAttribute('download', `clientes_export_${slug}.csv`);
            ...
            <div className="max-w-[1440px] mx-auto">
                <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4 mb-8">
                    ...
                    <div className="flex gap-3 items-center flex-wrap">
                        <button onClick={triggerImport} disabled={importing} className="px-4 py-2.5 rounded-xl border border-white/10 text-white/70 hover:bg-white/5 hover:text-white transition-colors text-sm font-medium flex items-center gap-2">
                            {importing ? <span className="w-4 h-4 border-2 border-white/30 border-t-white rounded-full animate-spin" /> : <Upload size={16} />}
                            Import CSV
                        </button>
                        <button onClick={handleExportCSV} className="px-4 py-2.5 rounded-xl border border-white/10 text-white/70 hover:bg-white/5 hover:text-white transition-colors text-sm font-medium flex items-center gap-2">
                            <Download size={16} />
                            Export CSV
                        </button>
